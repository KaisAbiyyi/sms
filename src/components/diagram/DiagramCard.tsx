...
const DiagramCard = (data: Array<DiagramCardProps>, id: string | number | undefined) => {
    ...
    const debug = data
        ...
        .map((item: DiagramCardProps) => {
            console.log(data.filter(filter => filter.parent === item.id).length > 1)
        })

    return data
        .filter(item => item.parent === id)
        .map((item: DiagramCardProps) => (
            <div className={`flex flex-col items-center justify-start flex-grow  `} key={item.id}>
                <div className="relative flex p-3 rounded-lg shadow w-72 bg-slate-50">
                    ...
                    {/* <div className="absolute top-0 w-1 h-8 -translate-y-full left-1/2 bg-slate-500"></div>
                    <div className="absolute bottom-0 w-1 h-8 translate-y-full left-1/2 bg-slate-500"></div> */}
                </div>
                <p>
                </p>
                {data.filter(filter => filter.parent === item.id).length > 1 && !LastChildrenIds.includes(item.id) ? <div className="w-4/6 h-6 m-auto bg-red-500"></div> : ''}
        ...
        <>
            <div className="p-4 m-auto max-w-5/6">
                <div className={"relative flex flex-col w-auto whitespace-nowrap flex-nowrap overflow-x-scroll " + scrollbarClass}>
                    <div className="relative flex p-3 m-auto rounded-lg shadow w-72 bg-slate-50">
                        <div className="relative w-24 overflow-hidden rounded-lg bg-slate-200">
                            {MainCard?.photo &&
                    ...
                    {/* <div className="w-4/6 h-10 m-auto bg-red-500"></div> */}
